...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
    ...
    ```bash
    make prepare NAME=fivetran REPOSITORY=github.com/HighTideCRM/pulumi-fivetran ORG=myorg
    ```
   ...
   - find and replace all instances of the boilerplate `myorg` with the `ORG` of your provider.
   - replace all instances of the `github.com/HighTideCRM/pulumi-fivetran` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-fivetran
  ...
  ignore: []
  ldflags:
  - -X github.com/HighTideCRM/pulumi-fivetran/provider/pkg/version.Version={{.Tag}}
